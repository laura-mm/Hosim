...
div = [] # gi, mi
for gi in range(6):
	d = np.loadtxt("div2_{0}.txt".format(gi), delimiter = ",")
	div.append(d)
	
crit = [] # gi, mi
for mi in range(12):
	c = np.loadtxt("crit2_{0}.txt".format(mi), delimiter = ",")
	crit.append(c)
...
for mi in range (1):
	...
	for gi in range(6):
		...
		plt.axvline(x = div[gi][mi], linestyle = ':', color = col[gi])
...
colour = np.loadtxt("allcolour2T.txt", delimiter=",")
...
for gi in range(6):
	...
	plt.scatter(muval, div[gi], marker = 'X', color = 'k')
